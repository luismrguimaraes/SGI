...
export class MySceneGraph {
    ...
    constructor(filename, scene) {
		...
		// Shaders
		this.selectedShaders = []; // nodes that will appear on the interface as eligible to have shaders
		this.nodeIDSelected = -1; // id of selected node on the interface
   ...
   */
    parseComponents(componentsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
			...
			// Add node to selectable shaders groups
            var nodeShaderSelectable = this.reader.getBoolean(children[i], 'selectedShaders', false);
            if(nodeShaderSelectable == true){
                this.selectedShaders.push(componentID);
            };
        }
