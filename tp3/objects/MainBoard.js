...
export class MainBoard extends Board{
    ...
    initPieces(){
        ...
        for (let i = 0; i < 3; i+=1){
            for (let j = ((i+1)%2); j < 8; j+=2){
                pieces.push(new Piece(this.scene, this, 1, 'piece ' + `${pieces.length}`, this.board[i][j]))
                this.board[i][j].set_isFree(false)
            }
        }
        for (let i = 7; i > 4; i-=1){
            for (let j = ((i+1)%2); j < 8; j+=2){
                pieces.push(new Piece(this.scene, this, 0, 'piece ' + `${pieces.length}`, this.board[i][j]))
                this.board[i][j].set_isFree(false)
            }
        }
    ...
    pick(piece_id){
        ...
        for (let i = 0; i < 8; i++){
            for (let j = 0; j < 8; j++){
                if (this.get(j, i).isFree){
                    this.get(j, i).setPickable(true)
                } else this.get(j, i).setPickable(false)
            }
        }
    }
    ...
    move(piece_id, x, y){
        ...
        for (let i = 0; i < 8; i++){
            for (let j = 0; j < 8; j++){
                this.get(j, i).setPickable(false)
            }
        }
    }
}